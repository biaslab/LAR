...
@model HAR(y, order=1, onoise=1.0) = begin
...
m_st = [mean(chain[Symbol("x[$i]")])[2][1] for i in 1:length(x)]
m_θ = [mean(chain[Symbol("θ1[$i]")])[2][1] for i in 1:length(x)]
